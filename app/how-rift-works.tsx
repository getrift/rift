...
function IconCell({ ids }: { ids: MarkId[] }) {
  return (
    <div className="flex flex-shrink-0 items-center gap-1.5 border-r border-border-hairline px-2.5 text-[#dfe3e8]">
      {ids.map((id) => (
...
function ChipBox({ ids, label }: { ids: MarkId[]; label: string }) {
  return (
    <div
      className={`${BOX} flex w-full items-stretch border-[rgba(255,255,255,0.1)] from-[#121315] to-[#0c0d0e] md:w-auto`}
    >
      <IconCell ids={ids} />
      <div className="flex flex-1 items-center px-3.5 py-2.5 md:flex-initial">
        <span className="font-mono text-[12px] leading-[15px] text-[#c8cdd6]">{label}</span>
      </div>
    </div>
  );
}
...
function Arrow() {
  return (
    <div className="flex flex-shrink-0 items-center justify-center self-center py-0.5 text-[rgba(255,255,255,0.42)] md:px-2 md:py-0">
...
export default function HowRiftWorks() {
  return (
    <figure className="not-prose !my-9 rounded-[14px] border border-border-hairline bg-[rgba(255,255,255,0.015)] px-5 py-11 md:relative md:left-1/2 md:!my-14 md:w-[760px] md:max-w-[92vw] md:-translate-x-1/2 md:px-6 md:py-16">
      ...
      <div className="flex flex-col items-stretch md:flex-row md:items-center md:justify-between">
        ...
        <div
          className={`${BOX} flex w-full flex-col items-center justify-center gap-2 border-[rgba(255,255,255,0.14)] from-[#131416] to-[#0c0d0e] px-7 py-4 shadow-[0_0_44px_-14px_rgba(255,255,255,0.32)] md:w-auto md:flex-shrink-0`}
        >
          <div className="flex items-center gap-2.5 text-[#f7f8f8]">
            <RiftMark size={40} />
            <span className="text-[22px] font-semibold tracking-tight">rift</span>
          </div>
          <span className="whitespace-nowrap font-mono text-[10px] tracking-[0.01em] text-[#8a8f98]">
            private · on your Mac
          </span>
        </div>
      ...
      <div className="relative mt-10 hidden h-4 md:block" aria-hidden>
        <div className="absolute inset-x-[12%] top-1/2 -translate-y-1/2 border-t border-dashed border-[rgba(255,255,255,0.16)]" />
        <svg
          viewBox="0 0 8 8"
          className="absolute left-[12%] top-1/2 h-2 w-2 -translate-x-1/2 -translate-y-1/2 text-[rgba(255,255,255,0.3)]"
          fill="currentColor"
        >
          <path d="M4 0 L0 6 L8 6 Z" />
        </svg>
        <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-bg-app px-3 font-mono text-[9.5px] uppercase tracking-[0.1em] text-text-muted">
          served back to the same tools
        </span>
      </div>
      <figcaption className="mt-7 text-center font-mono text-[9.5px] uppercase tracking-[0.1em] text-text-muted md:hidden">
        ↺ served back to the same tools
      </figcaption>
    </figure>
  );
